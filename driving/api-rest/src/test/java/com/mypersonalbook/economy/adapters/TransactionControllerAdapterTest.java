...
public class TransactionControllerAdapterTest {
  ...
  @Mock
  private SaveTransactionUseCasePort saveExpenseUseCase;
  @Mock
  private DeleteTransactionUseCasePort deleteExpenseUseCase;
  @Mock
  private GetTransactionUseCasePort getExpenseUseCase;
  @Mock
  private GetTransactionsUseCasePort getExpensesUseCase;
  @Mock
  private ModifyTransactionUseCasePort modifyExpenseUseCase;

  @BeforeEach
  void setUp() {
    this.transactionControllerAdapter = new TransactionControllerAdapter(
  ...
  void shouldReturn200StatusCode_WhenPostExpense() {
    ...
    final ResponseEntity<Void> RESULT = this.transactionControllerAdapter
        .postTransaction(TRANSACTION_REQUEST_BODY_TYPE());
  ...
  void shouldReturn204StatusCode_WhenDeleteExpense() {
    ...
    final ResponseEntity<Void> RESULT = this.transactionControllerAdapter.deleteTransaction(TRANSACTION_ID_1);
  ...
  void shouldReturn200StatusCode_WhenGetExpense() {
    ...
    final ResponseEntity<TransactionResponseType> RESULT = this.transactionControllerAdapter
        .getTransaction(TRANSACTION_ID_1);
  ...
  void shouldReturn200StatusCode_WhenGetExpenses() {
    ...
    final ResponseEntity<TransactionsResponseType> RESULT = this.transactionControllerAdapter.getTransactions(
  ...
  void shouldReturn200StatusCode_WhenPatchExpense() {
    ...
    final ResponseEntity<Void> RESULT = this.transactionControllerAdapter.patchTransaction(
        TRANSACTION_ID_1, CATEGORY_EXPENSE_TRANSACTION_TYPE_LOWER_CASE, TRANSACTION_REQUEST_BODY_PATCH_TYPE());
    assertEquals(HttpStatus.OK, RESULT.getStatusCode());
  }
}
